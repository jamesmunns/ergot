...
async fn main(spawner: Spawner) {
    ...
    let led_pins = [
        ...
        // Output::new(p.P0_15, Level::High, OutputDrive::Standard), // row3
        Output::new(p.P0_30, Level::High, OutputDrive::Standard), // col5
    ];
    let _row3 = Output::new(p.P0_15, Level::High, OutputDrive::Standard);
    let btn_pins = [Input::new(p.P0_14, Pull::Up), Input::new(p.P0_23, Pull::Up)];
    let names = ["LED1", "LED2"];
    ...
    for ((idx, name), btn) in names.iter().enumerate().zip(btn_pins.into_iter()) {
        spawner.must_spawn(button_worker(btn, name, idx as u8)); // only 2 btns
    }
...
#[task(pool_size = 4)]
async fn button_worker(mut btn: Input<'static>, name: &'static str, idx: u8) {
    let client = STACK
        .endpoints()
    ...
    loop {
        ...
        let _ = STACK
            .topics()
            .broadcast::<ButtonPressedTopic>(&(idx + 1), None);
